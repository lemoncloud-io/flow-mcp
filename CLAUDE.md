...
- **Shared graph rules — `@lemoncloud/flow-engine`**: the headless engine from `eureka-flow` owns the rules this server used to hand-roll. `ws-client.routeFrame` folds socket frames through `parseSocketFrame` + `reduceNodeEvent`/`reducePortEvent` (sequence high-water mark, state priority, per-run reset, port-shaped ids); `node_get` decodes the wire arrays with `mergeNodeView`; `flow_graph`/`flow_export` collapse duplicate edges with `deduplicateEdges`; `isWritableNodeId` guards the id charset `core/ids.ts` documents. Two deliberate exceptions: `SKIPPED` is terminal here but unmodelled by the engine, so unranked states fall back to last-write (`acceptsState`), and the port-value merge / downstream propagation in `core/ingress.ts` is **not** adopted — this server passes the `ports` array through instead.
